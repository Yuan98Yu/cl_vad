...
def save_checkpoint(state, is_best=0, gap=1, filename='models/checkpoint.pth.tar', keep_all=False):
    ...
    if is_best:
        ...
        past_best = sorted(past_best, key=lambda x: int(
...
def load_checkpoint(args, model_without_ddp, optimizer):
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            print("[Warning] no checkpoint found at '{}', use random init".format(
                args.resume))
        ...
        if os.path.isfile(args.pretrain):
            ...
            print("=> no checkpoint found at '{}', use random init".format(
                args.pretrain))
...
class Logger(object):
    ...
    def log(self, string):
        with open(self.filepath, 'a') as f:
            ...
            f.write(
                strfdelta(time_stamp, "{d}-{h:02d}:{m:02d}:{s:02d}")+'\t'+string+'\n')
